...
e.g. §R only exists if `/spec-research` ran) but is never reordered.
...
## §R RESEARCH
optional. external-knowledge log. pipe table. present only if `/spec-research` ran.
...
|---|---|---|
| `/spec-grill` | sharpens | §G + §C (proposes; writes on OK) |
| `/spec new` | creates | all |
| `/spec amend` | edits | named only |
| `/spec bug` | appends | §B + §V |
| `/spec-research` | appends | §R |
| `/spec-review` | hardens | §V (+ risk report, no silent rewrite) |
| `/spec-build` | flips | §T status cell `.` → `~` → `x` |
| `/spec-deepen` | proposes | §I + §V + §T (writes on OK) |
| `/spec-check` | — | read only |
...
Ceremony scales to blast radius, never to ego. One-line fix → just `/spec-build`.
New feature in a shared module → `/spec-grill` then `/spec-review` first. The full
spec-grill→spec→spec-research→spec-review→spec-build chain is for genuinely uncertain or
